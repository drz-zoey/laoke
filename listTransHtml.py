...
# 正则表达式：匹配品牌
brand_pattern = re.compile(r'兰蔻|雅诗兰黛|卡地亚|圣罗兰|蒂佳婷|cpb肌肤之钥|海蓝之谜|娇韵诗|阿玛尼|赫莲娜|科颜氏|Gucci|倩碧|黛珂|资生堂|馥蕾诗|欧莱雅|迪奥|莱珀妮|赫莲娜|娇兰|SK-II|植村秀|Tom Ford|NARS|MAC|魅可|whoo后|雅萌|ahc|飞利浦|百瑞德|兰芝|香奈儿|银座|衰败城市|伊菲丹|芭比布朗|祖玛龙|希思黎|碧欧泉|欧舒丹|修丽可|爱马仕|蔻依|祖玛珑|梅森马吉拉')

#根据单品名称匹配品牌
brand_match_skii_pattern = re.compile(r'(SK2|sk2|skii|sk-ii|眼面套|神灯套|小灯泡|神仙水|大红瓶)')
brand_match_ysld_pattern = re.compile(r'(增量五件套|抗衰|红石榴|小棕瓶|抗蓝光|眼绷带|白金眼霜|白金粉底液|沁水|dw|原生液|樱花水|智妍|抗衰老|黑松露|白金面霜)')
brand_match_hlzm_pattern = re.compile(r'(LAMER|Lamer|lamer|精粹水|精萃乳|海南之谜|舒缓喷雾|浓缩精华|经典面霜)')
brand_match_zst_pattern = re.compile(r'(安耐晒|安热沙|蓝胖子|红腰子|悦薇)')
brand_match_cpb_pattern = re.compile(r'(cpb|肌肤之钥|CPB|Cpb)')
brand_match_whoo_pattern = re.compile(r'(whoo)')
brand_match_tf_pattern = re.compile(r'(TF|Tf|tf|tom ford|Tom Ford|TOM FORD|乌木)')
...
brand_match_msmjl_pattern = re.compile(r'(马吉拉|梅森马吉拉)')
brand_match_lk_pattern = re.compile(r'(小白管|菁纯|小黑瓶|发光眼霜|雪花霜|粉水|极光)')
brand_match_kys_pattern = re.compile(r'(金盏花|宝宝霜|高保湿|白泥|紫波A面霜)')
...
brand_match_ysl_pattern = re.compile(r'(圣罗兰|YSL|ysl|Ysl|夜皇后|藏金)')
brand_match_gucci_pattern = re.compile(r'(Gucci|gucci|古驰)')
brand_match_hr_pattern = re.compile(r'(Hr|HR|赫莲娜|黑绷带|白绷带|绿宝瓶|黑白绷带|小针管)')
