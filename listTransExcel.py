...
input_file = path + "\\example_test1.txt"
...
with open(input_file, 'r' , encoding='utf-8') as f:
    ...
    for line in f:
        ...
        # 提取出现的品牌名称
        brand_match = re.search(r"(兰蔻|卡诗|雅诗兰黛|卡地亚|玫珂菲|圣罗兰|蒂佳婷|cpb肌肤之钥|海蓝之谜|娇韵诗|阿玛尼|赫莲娜|科颜氏|Gucci|倩碧|黛珂|资生堂|馥蕾诗|欧莱雅|迪奥|莱珀妮|赫莲娜|娇兰|SK-II|植村秀|Tom Ford|NARS|MAC|魅可|whoo后|雅萌|ahc|飞利浦|百瑞德|兰芝|香奈儿|银座|菲拉格慕|衰败城市|伊菲丹|芭比布朗|祖玛龙|希思黎|碧欧泉|欧舒丹|修丽可|爱马仕|蔻依|祖玛珑|梅森马吉拉)", line)
       
        #根据单品名称匹配品牌
        brand_match_skii = re.search(r"(SK2|sk2|skii|sk-ii|skll|晶莹露|眼面套|神灯套|小灯泡|神仙水|大红瓶|前男友|甄选三件套|增选3件套)", line)
        brand_match_ysld = re.search(r"(增量五件套|抗衰|红石榴|小棕瓶|蓝光眼霜|抗蓝光|眼绷带|白金眼霜|白金粉底液|沁水|dw|原生液|樱花水|智妍|抗衰老|黑松露|白金面霜)", line)
        brand_match_hlzm = re.search(r"(LAMER|Lamer|lamer|绿眼霜|精粹水|精粹乳|精萃乳|海南之谜|舒缓喷雾|浓缩精华|经典面霜)", line)
        brand_match_zst = re.search(r"(安耐晒|安热沙|蓝胖子|红腰子|悦薇|百优|百忧)", line)
        brand_match_cpb = re.search(r"(cpb|肌肤之钥|CPB|Cpb|长管隔离)", line)
        brand_match_whoo = re.search(r"(whoo|天气丹|天率丹|拱辰享|津率享)", line)
        ...
        brand_match_lk = re.search(r"(小白管|菁纯|箐纯|小黑瓶|大眼精华|发光眼霜|肌底液|雪花霜|粉水|极光|持妆|是我|三件套带水|带水三件套|小蛮腰)", line)
        brand_match_kys = re.search(r"(金盏花|宝宝霜|高保湿|白泥|紫波A面霜|爱海南三件套|淡斑精华)", line)
        ...
        brand_match_ysl = re.search(r"(圣罗兰|YSL|ysl|Ysl|yls|夜皇后|藏金|粉气垫|皮气垫|小金条|小黑条|小粉条)", line)
        ...
        brand_match_ams = re.search(r"(爱马仕|大地)", line)
        brand_match_xne = re.search(r"(香奈儿|channel|蔚蓝|山茶花洁面|山茶花|嘉伯丽尔|黄邂逅|粉邂逅|绿邂逅)", line)
        brand_match_jys = re.search(r"(娇韵诗|双萃|弹簧水)", line)
        brand_match_lyw = re.search(r"(罗意威|Loewe|loewe)", line)
        brand_match_boq = re.search(r"(碧欧泉|水动力)", line)
        brand_match_lbn = re.search(r"(莱伯妮|莱珀妮|LP|La Prairie)", line)
